...
I used to say "terminal" or "shell" interchangably, but they're not the same thing.
...
In this examples, both of those print on the screen, so you can't tell the difference.
But you can redirect them separately...
...
Pipes are awesome! They can be thought of as simply a way to string commands together to make bigger commands.

    echo "hello\nthere\nfaithful\nfriend" # outputs several lines
    echo "hello\nthere\nfaithful\nfriend" | grep 'e'
    echo "hello\nthere\nfaithful\nfriend" | grep -v 'e'
    echo "hello\nthere\nfaithful\nfriend" | grep -v 'e' | cut -b 3-8
...
If these didn't run in parallel, you'd have to stop the `tail` process somehow so that `grep` could get the output and filter out what you want, but since they're parallel, you can get results in real time.
...
See http://blog.honeybadger.io/ruby-guide-environment-variables/ and http://nathanmlong.com/2014/02/understanding-unix-environment-variables/ for more about environment variables and how they're passed between processes.
...
All of these happen before running it, so you can stick `echo` in front to see what they do; after the expansion it just finds `echo` with some arguments and runs it.
    ...
    echo touch foo{1,2,3}.txt # expands to  `echo touch foo1.txt foo2.txt foo2.txt`
    echo rm foo.*             # expands to  `echo rm foo.txt foo.rb` (if those exist)
...
If you save this as `timestamp` in a folder on your `$PATH`:
    ...
    #!/usr/bin/env ruby
    # This program is dumb because `date` is already built in....
    require "date"
    puts Date
...
- http://nathanmlong.com/2013/01/making-vim,-unix-and-ruby-sing-harmony/
...
`sed` can do a ton more stuff, and `awk` can also do a ton of stuff - they are their own programming languages!

If you know Ruby, you can use it instead:

    seq 1 4 | ruby -ne 'puts $_ if Integer($_) % 2 == 0' # outputs 2 and 4
    echo nathan | ruby -pe '$_.upcase!' # outputs NATHAN (must mutate $_ to see)
...
- Environment variables - featuring especially $PWD, $PATH, and Ruby-specific ones like $GEM_PATH and $GEM_HOME
- There's nothing special about flags, they're just arguments that are parsed differently if the program wants - `ls -l .git` just has two args (it knows pwd too)

## Stuff I should learn better

- "Operating as the superuser" vs being able to; what changes when I type "sudo"? What can superuser do that others can't? What is the mechanism controlling this? How are file permissions enforced, for example?
  - Also, shouldn't my command prompt show whether I'm operating as superuser right  now?
